...
void
loadcomList (string0 bufferx, int px)
{
  bool svs, justone;
  int theone;
  register int i;
  ...
  if (!erred)
      ...
      if (!erred)
	{
      ...
      if (!erred)
	{
	      Resetx2 (&t, filen.A, sizeof (filen.A));
	      fprintf (output.fp, "save[");
	      if (svs) // svar format schur:=[[sfn], optional_code, var1, var2,...]
		{
		  s = fgets (wd,MAXSTRING,t.fp);
		  if (s==NULL || strcmp(wd,"schur:=[[sfn],", strlen("schur:=[[sfn],"))!=0)
		  {
		     fprintf(stderr,"Wrong file format for %s ? See sb_listoutput\n", filen.A);
		     error (MISTAKE, 1);
		     return;
		  }
		  s += strlen("schur:=[[sfn],");
		...
		{		// rvar.. format : [[rep, groupe], var1, var2, ...]
		  ...
		  for (i = 1; i <= rvarlimit; i++)
			  ...
			  || (justone && i == theone))
			{
			  ...
			  fprintf (t.fp, ",");
			  saveframe2 (&t, cptr->conval);
			  fprintf (t.fp, ",\"%c\"", cptr->conlab);
			      fprintf (t.fp, ",");
			      saveframe2 (&t, cptr->val);
